...
export interface Env {
  ...
  HN_FIREBASE: string; HN_ALGOLIA: string;
...
const SERVER_INFO = { name: "hn-trending-mcp", version: "0.1.0" };
...
const LLMS_TXT = `# hn-trending-mcp

> Hacker News front-page + full-text search + recent Show HN tracker.

## Tools
- hn_top(kind?, limit?) — current top / best / new / ask / show / job stories
- hn_search(query, tags?, min_points?, days_ago?)
- hn_show_recent(min_points?, days_ago?) — Show HN launches
- hn_item(id)
- hn_user(handle)

Endpoint: https://hn-trending-mcp.prakhar-cognizance.workers.dev/mcp
`;
const LANDING = `<!doctype html><html><head><meta charset="utf-8"><title>hn-trending-mcp</title>
...
<body><h1>hn-trending-mcp</h1><p>Hacker News front-page + full-text search + recent Show HN tracker.</p>
