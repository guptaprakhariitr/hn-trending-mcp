# Hacker News Trending MCP — Front-page stories, Algolia full-text, Show HN launch tracker
...
[![Live](https://img.shields.io/badge/live-hn-trending-mcp.atlasword.workers.dev-brightgreen.svg)](https://hn-trending-mcp.atlasword.workers.dev)

> HN front page, full-text Algolia search, Show HN launch tracker, item & user lookup

Hosted MCP server over Hacker News. Pull current front-page / best / new / Ask HN / Show HN / job stories, run full-text Algolia search across stories and comments with point/age filters, track recent Show HN launches, and fetch individual items or user profiles. For tech-trend, indie-launch, and Show-HN-watcher LLMs.
...
- **Live MCP endpoint:** `https://hn-trending-mcp.atlasword.workers.dev/mcp`
...
- **Health:** `GET https://hn-trending-mcp.atlasword.workers.dev/health` → `{ok: true, ts: <unix>}`
- **Status:** `https://mcp-hub.atlasword.workers.dev/status.json` (portfolio rollup)

Sample `tools/list` request:

```bash
curl -sS -X POST https://hn-trending-mcp.atlasword.workers.dev/mcp \
...
|---|---|---|
| `hn_top` | `kind?, limit?` | HN front-page stories — kind=top|best|new|ask|show|job. Up to 100 items. |
| `hn_search` | `query, tags?, min_points?, days_ago?` | Full-text search across stories + comments via Algolia. |
| `hn_show_recent` | `min_points?, days?` | Recent Show HN launches with at least N points in the last K days. |
| `hn_item` | `id` | Fetch a single HN item by id (story or comment). |
| `hn_user` | `username` | HN user profile: karma, account age, recent submissions. |
...
```bash
npx -y mcp-remote https://hn-trending-mcp.atlasword.workers.dev/mcp \
  --header "Authorization: Bearer mck_YOUR_API_KEY"
```

Get a key at **https://hn-trending-mcp.atlasword.workers.dev/upgrade?tier=solo** (see [Getting an API key](#getting-an-api-key)).
...
1. Visit `https://hn-trending-mcp.atlasword.workers.dev/upgrade?tier=solo` (or `tier=team` / `tier=pro`).
...
4. You land on `https://hn-trending-mcp.atlasword.workers.dev/welcome?key=<api_key>` — copy the key now (it is only displayed once at this URL).
5. Paste the key into Cursor / Claude Desktop config (see above).
6. View / rotate / export the account at `https://hn-trending-mcp.atlasword.workers.dev/account` (Bearer-auth).
...
- **Hacker News API (Firebase)** — https://github.com/HackerNews/API — *Public API*
- **HN Algolia Search** — https://hn.algolia.com/api — *Free public-API*
...
- **Data export:** `GET https://hn-trending-mcp.atlasword.workers.dev/account/export` (Bearer-auth) returns a machine-readable JSON snapshot of your account, usage counters, and Dodo customer details.
...
- **Source repo:** [https://github.com/guptaprakhariitr/hn-trending-mcp](https://github.com/guptaprakhariitr/hn-trending-mcp)
...
The npm package is just a launcher — it shells out to [`mcp-remote`](https://www.npmjs.com/package/mcp-remote) and points it at the hosted endpoint (`https://hn-trending-mcp.atlasword.workers.dev/mcp`).
